    ...
    def _path_to_root(self, frame: str) -> list[str]:
        ...
        while current != self.root:
            if current not in self._parent:
                raise KeyError(f"프레임 {frame!r}은 root {self.root!r}에 연결되어 있지 않습니다")
            current = self._parent[current]
            if current in visited:
                raise KeyError(f"프레임 체인에 순환이 있습니다: {current!r}")
            visited.add(current)
            path.append(current)
        return path
...
def default_chain() -> CoordinateChain:
    ...
    T_base_link = make_T(rot_z(angle), [0.35, 0.05, 0.45])
    T_link_camera = make_T(
        rot_y(-angle) @ rot_x(np.deg2rad(67.5)),
        [0.12, 0.04, 0.18],
    )
