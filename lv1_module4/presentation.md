...
추가로 전체 점의 5%인 **12개**를 법선 방향으로 0.2~0.5 m 이동시켜 이상치를 만들었다. 초기 최소제곱 평면의 잔차를 MAD로 걸러내고, 남은 점으로 평면을 다시 피팅하는 과정을 반복했다. MAD(엠에이디)는 중앙값 주변에서 잔차가 얼마나 퍼져 있는지 재는 값이어서 큰 이상치의 영향을 덜 받는다. 실제 이상치 위치는 검출률 평가에만 사용했다.

| 비교 항목 | 실행 결과 |
|---|---:|
| Kabsch 회전 오차, 제거 전 | 2.415787° |
| Kabsch 회전 오차, 제거 후 | **0.184601°** |
| 제거한 이상치 | 12 / 12 |
| 잘못 제거한 정상점 | 0 / 228 |
| 정제 후 평면 법선과 참값의 각도 | 7.358360° |
...
- 반복 MAD 정제는 초기 평면과 점 분포에 영향을 받으므로 항상 오차가 줄지는 않는다. 별도 난수 실험(시드 999, 이상치 12개)에서는 이상치 11개를 제거하고도 회전 오차가 1.532958°에서 1.978188°로 증가했다. 위 본 실험의 시드 42 결과와 구분한다.
...
시작 위치는 (0.20, −0.30, 0.65), 목표 위치는 (0.732846, −0.398734, 0.843400) m였다. 자세는 SLERP, 위치는 자연 스플라인으로 동시에 보간했다. 프레임 간 회전각의 평균은 **2.346413°**, 표준편차는 **4.373310e−13°**였다.

![자세·가속도 품질](images/03_cell28.png)

인접 표본의 가속도 차이 최댓값은 스플라인 0.228015, 선형 보간 11.227826이었다. 이것은 표본 간 변화량이며, 수학적인 불연속 크기와 구분해야 한다. 스플라인 계수로 직접 계산한 경유점 좌우 가속도 차이는 **2.553513e−15**로 0에 가까웠다.
