...
def matrix_to_quaternion(R) -> np.ndarray:
    ...
    if trace > 0.0:
        s = 2.0 * np.sqrt(max(0.0, 1.0 + trace))
        w = 0.25 * s
        x = (R[2, 1] - R[1, 2]) / s
        y = (R[0, 2] - R[2, 0]) / s
        z = (R[1, 0] - R[0, 1]) / s
    ...
    length = float(np.sqrt(q @ q))
    if length <= np.finfo(float).eps:
        raise ValueError("유효한 회전행렬에서 쿼터니언을 만들 수 없습니다")
    q /= length
    if q[3] < 0.0:
        q = -q
    return q
...
def quaternion_to_matrix(q) -> np.ndarray:
    ...
    if q.shape != (4,):
        raise ValueError(f"쿼터니언 shape은 (4,)여야 합니다. 받은 shape={q.shape}")
    length = float(np.sqrt(q @ q))
    if length <= np.finfo(float).eps:
...
def quat_angle(q0, q1) -> float:
    ...
    if q0.shape != (4,) or q1.shape != (4,):
        raise ValueError("q0와 q1은 모두 shape (4,)여야 합니다")
    n0 = float(np.sqrt(q0 @ q0))
    n1 = float(np.sqrt(q1 @ q1))
    if n0 <= np.finfo(float).eps or n1 <= np.finfo(float).eps:
        raise ValueError("영 쿼터니언은 비교할 수 없습니다")
    cosine = float(np.clip(abs((q0 / n0) @ (q1 / n1)), 0.0, 1.0))
...
def slerp(q0, q1, t: float, eps: float = 1e-8) -> np.ndarray:
    ...
    return result / np.sqrt(result @ result)
