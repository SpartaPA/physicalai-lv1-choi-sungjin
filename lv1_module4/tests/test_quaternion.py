...
def test_matrix_quaternion_roundtrip(rng):
    for _ in range(50):
        ...
        assert np.allclose(quaternion_to_matrix(matrix_to_quaternion(R)), R)


def test_slerp_nearly_identical_poses(q_pair):
    q0, _ = q_pair
    q1 = matrix_to_quaternion(quaternion_to_matrix(q0) @ rot_z(1e-10))
    middle = slerp(q0, q1, 0.5)
...
def test_lerp_norm_drops_below_one(q_pair):
    q0, q1 = q_pair
    assert np.linalg.norm(lerp_quat(q0, q1, 0.5)) < 1.0
    assert np.isclose(np.linalg.norm(lerp_quat(q0, q1, 0.5, normalize=True)), 1.0)
