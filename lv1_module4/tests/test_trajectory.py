...
def test_spline_velocity_is_smoother_than_linear():
    t = np.linspace(T_WP[0], T_WP[-1], 501)
    linear_velocity = finite_diff(linear_interp(T_WP, Q_WP_1D, t), t)
    spline_velocity = finite_diff(cubic_spline_interp(T_WP, Q_WP_1D, t), t)
    assert np.max(np.abs(np.diff(linear_velocity))) > 5 * np.max(np.abs(np.diff(spline_velocity)))
