...
이미 클론한 저장소가 있다면 다시 클론하지 않고 그 안의 `lv1_module4`로 이동한다. 가상환경은 Git에 포함되지 않으므로 새 PC에서 별도로 만든다.
...
5. **5-5**에서 이상치를 **12개(5%)** 넣고 제거 전후를 비교한다. 이번 결과는 **12개 제거, 정상점 손실 0개, 회전 오차 2.416° → 0.185°**였다.
...
현재 저장한 실행 결과는 **pytest 44개 통과**, 노트북 검증 **176개 통과 / 실패 0개**, GIF **60프레임**이다. 전체 검증 명령의 마지막에는 `노트북 3개 전체 실행 및 저장 결과 검사 통과`가 나온다.
...
이 실습은 합성 점군과 오프라인 궤적을 사용한다. 실제 로봇의 충돌·관절 제한·센서 지연까지 검증한 것은 아니다. 기록된 본 실행은 Ubuntu 22.04 WSL에서 수행했다. 이전 2026-09-09 검증에서는 같은 WSL의 빈 가상환경에서도 `requirements.txt` 설치, 주요 패키지 가져오기, 당시 함수 테스트 43개 통과와 `pip check`의 의존성 충돌 없음을 확인했다. 이번 12개 이상치 조건을 포함한 44개 테스트는 기존 가상환경에서 실행했다. 별도 물리 PC나 Windows Python에서의 실행과 구분한다.
